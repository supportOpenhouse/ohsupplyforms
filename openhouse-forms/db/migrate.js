...
DO $$ BEGIN
  ...
  -- Gmail thread id + message id of the Form 3 (token request) email — consumed by the
  -- separate transaction-management process. Deliberately NOT email_thread_id /
  -- email_message_id, which are the Form 4/6/9 reply-chain and must stay untouched.
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='properties' AND column_name='txn_mgmt_message_id') THEN ALTER TABLE properties ADD COLUMN txn_mgmt_message_id TEXT; END IF;
